...
<div class="container-fluid text-white">
    ...
    <div class="row">
        <div class="col-md-8 col-lg-8 col-12">
            <form method="POST" action="{{url('leader/editprocess/'.$ld->id)}}">
                @method('PATCH')
                @csrf
                <div class="form-group">
                  <label>Nama Pegawai</label>
                  <input name="name" type="text" class="form-control @error('judul') is-invalid @enderror" value="{{old('name', $ld->name)}}" placeholder="Masukan nama Pimpinan / Atasan" required>
                  <small>Petunjuk : Isi dengan Nama Atasan (Nama dan Gelar)</small>
                </div>
                <div class="form-group">
                  <label>NIP</label>
                  <input name="nip" type="text" class="form-control" placeholder="Masukan NIP Pimpinan / Atasan" value="{{old('nip', $ld->nip)}}" required>
                ...
                <div class="form-group">
                    ...
                    <select class="custom-select" name="organization_id" autocomplete="on">
                        ...
                        @foreach ($org as $item)
                        <option value="{{$item->id}}" {{ old('organization_id', $ld->organization_id) == $item->id ? 'selected' : null}}>{{ $loop->iteration." | ".$item->title}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                  <label>Jabatan</label>
                  <input name="job" type="text" class="form-control" placeholder="Masukan Jabatan Atasan" value="{{old('job', $ld->job)}}" required>
                  <small>Petunjuk : Isi dengan Jabatan Atasan / Pimpinan</small>
                </div>
                <div class="row">
                    <div class="col">
                        <button type="submit" class="btn btn-primary btn-block">Simpan</button>
                        <a href="{{ url('leader')}}" class="btn btn-primary mb-4 btn-block">Batal</a>
                    </div>
                </div>
            </form>
        </div>
