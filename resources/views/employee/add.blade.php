...
<div class="container-fluid text-white">
    ...
    <div class="row">
        <div class="col-md-8 col-lg-8 col-12">
            <form method="POST" action="{{url('/emp/addprocess')}}">
                @method('PATCH')
                @csrf
                <div class="form-group">
                  <label>Nama</label>
                  <input name="name" type="text" class="form-control @error('name') is-invalid @enderror" placeholder="Masukan Nama Anda" required value="{{old('name')}}" required>
                ...
                <div class="form-group">
                  <label>NIP</label>
                  <input name="nip" type="text" class="form-control @error('nip') is-invalid @enderror" placeholder="Masukan NIP Anda/ Kosongkan jika belum ada" value="{{old('nip')}}">
                ...
                <div class="form-group">
                  <label>Jabatan</label>
                  <input name="job" type="text" class="form-control" placeholder="Masukan Jabatan dan Tugas Tambahan jika ada" value="{{old('job')}}" required>
                ...
                <div class="form-group">
                    ...
                    <textarea name="contact" type="text" class="form-control">{{old('contact')}}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="custom-select" name="leader_id" autocomplete="on" required>
                        ...
                        @foreach ($ld as $item)
                        <option value="{{ $item->id}}">{{ $loop->iteration." | ".$item->name." - ".$item->job." - ".$item->organization->title}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="custom-select" name="unit_id" required>
                        ...
                        <option value="{{ $items->id}}">{{ $loop->iteration." | ".$items->title." - ".$items->organization->title}}</option>
                        @endforeach
                    </select>
