...
<div class="container-fluid text-white">
    ...
    <div class="row">
        <div class="col-md-8 col-lg-8 col-12">
            <form method="POST" action="{{ url('/user/passaccount')}}">
                @method('PATCH')
                @csrf
                <div class="form-group">
                  <label>Masukan Password Baru</label>
                  <input name="password" type="text" class="form-control" placeholder="Masukan Password Baru" value="{{ old('password') }}">
                  <small>Petunjuk : isi dengan password baru anda | kosongkan untuk reset ke password awal</small>
                </div>
                <div class="row">
                    <div class="col">
                        <button type="submit" class="btn btn-primary btn-block">Simpan Perubahan</button>
                        <a href="{{ url('/dashboard')}}" class="btn btn-danger btn-block mb-4">Batal</a>
                    </div>
                </div>
            </form>
        </div>
